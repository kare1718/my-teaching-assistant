...
router.get('/transactions', async (req, res) => {
  ...
  const rows = await getAll(
    'SELECT * FROM sms_transactions WHERE academy_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?',
    [req.academyId, limit, offset]
  );

  const countRow = await getOne('SELECT COUNT(*) as total FROM sms_transactions WHERE academy_id = ?', [req.academyId]);
...
router.post('/send', async (req, res) => {
  ...
  // TODO: SOLAPI API 호출
  // const solapiResult = await sendViaSolapi(recipients, message, smsType);
